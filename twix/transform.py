...
def transform(pdf_paths, result_folder_path):
    total_cost = 0
    phrases, cost = extract.extract_phrase(pdf_paths, result_folder_path)
    total_cost += cost 
    fields, cost = key.predict_field(pdf_paths, result_folder_path)
    total_cost += cost 
    template, cost = pattern.predict_template(pdf_paths, result_folder_path)
    total_cost += cost 
